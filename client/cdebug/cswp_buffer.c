...
int cswp_buffer_put_uint32(CSWP_BUFFER* buf, uint32_t val)
{
    ...
    __CSWP_PUT_BYTE(buf, ((uint8_t)(val & 0xFF)));
    __CSWP_PUT_BYTE(buf, ((uint8_t)((val >> 8) & 0xFF)));
    __CSWP_PUT_BYTE(buf, ((uint8_t)((val >> 16) & 0xFF)));
    __CSWP_PUT_BYTE(buf, ((uint8_t)((val >> 24) & 0xFF)));
...
int cswp_buffer_put_uint64(CSWP_BUFFER* buf, uint64_t val)
{
    ...
    __CSWP_PUT_BYTE(buf, ((uint8_t)((val >> 16) & 0xFF)));
    __CSWP_PUT_BYTE(buf, ((uint8_t)((val >> 24) & 0xFF)));
    __CSWP_PUT_BYTE(buf, ((uint8_t)((val >> 32) & 0xFF)));
    __CSWP_PUT_BYTE(buf, ((uint8_t)((val >> 40) & 0xFF)));
    __CSWP_PUT_BYTE(buf, ((uint8_t)((val >> 48) & 0xFF)));
    __CSWP_PUT_BYTE(buf, ((uint8_t)((val >> 56) & 0xFF)));
    buf->used = buf->pos;
    return CSWP_SUCCESS;
}
...
int cswp_buffer_put_varint(CSWP_BUFFER* buf, varint_t val)
{
    ...
    while (val > 0x7F)
    {
        __CSWP_PUT_BYTE(buf, ((uint8_t)(0x80 | (val & 0x7F))));
        val >>= 7;
    }
    __CSWP_PUT_BYTE(buf, ((uint8_t)val & 0x7F));
    buf->used = buf->pos;
    return CSWP_SUCCESS;
}
...
int cswp_buffer_get_uint32(CSWP_BUFFER* buf, uint32_t* val)
{
    ...
    v |= ((uint32_t)(__CSWP_GET_BYTE(buf)) << 8);
    v |= ((uint32_t)(__CSWP_GET_BYTE(buf)) << 16);
    v |= ((uint32_t)(__CSWP_GET_BYTE(buf)) << 24);
    *val = v;
    return CSWP_SUCCESS;
}
